                  ...
                  <TableBody>
                    {contrato.itens && contrato.itens.length > 0 ? (
                      contrato.itens.map((item) => {
                        const itemSaldo = saldo.itensSaldo.find(saldoItem => saldoItem.id === item.id);
                        const quantidadeOriginal = itemSaldo?.quantidadeOriginal || item.quantidade || 0;
                        const quantidadeConsumida = itemSaldo?.quantidadeConsumida || 0;
                        const saldoQuantidade = itemSaldo?.saldoQuantidade || quantidadeOriginal;
                        const percentualConsumido = itemSaldo?.percentualConsumido || 0;
                        const valorUnitario = itemSaldo?.valorUnitario || item.valorUnitario || 0;
                        const totalItem = itemSaldo ? Math.max(0, itemSaldo.saldoValor) : (quantidadeOriginal * valorUnitario);

                        return (
                          <TableRow key={item.id}>
                            <TableCell className="font-medium">{item.descricao}</TableCell>
                            <TableCell>{quantidadeOriginal.toLocaleString('pt-BR')}</TableCell>
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <span className={quantidadeConsumida > 0 ? "text-red-500" : "text-muted-foreground"}>
                                  {quantidadeConsumida.toLocaleString('pt-BR')}
                                </span>
                            ...
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <span className={saldoQuantidade < quantidadeOriginal * 0.2 ? "text-red-500 font-bold" : "text-green-600 font-medium"}>
                                  {saldoQuantidade.toLocaleString('pt-BR')}
                                </span>
                                {saldoQuantidade < quantidadeOriginal * 0.2 && saldoQuantidade > 0 && (
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-bold text-green-600">
                    {saldo.itensSaldo.length > 0 
                      ? Math.max(0, saldo.itensSaldo
                          .reduce((total, itemSaldo) => total + itemSaldo.saldoValor, 0))
